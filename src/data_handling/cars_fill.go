...
func FillDuckDBWithCars(db SQLOverhead) {
	/*fill duckdb with cars data - COPY FROM CSV (nejrychlejší)*/
	query := `
    COPY raw_sales (
        vin, year_int, make, model, trim, body, transmission,
        color, interior, selling_price, mmr, seller, odometer, condition
    )
    FROM './data/cars_clean.csv'
    (
        FORMAT CSV,
        HEADER true,
        QUOTE '"'
...
func FillPostgresWithCars(db SQLOverhead) {
	...
	for {
		...
		if id, exists := yearMap[rawYear]; exists {
			...
			err := tx.QueryRow("INSERT INTO dim_year(year_int) VALUES($1) ON CONFLICT (year_int) DO UPDATE SET year_int=EXCLUDED.year_int RETURNING year_id", yVal).Scan(&yearID)
			if err != nil {
				log.Fatal(err)
			}
